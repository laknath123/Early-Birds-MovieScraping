...
# In[10]:
def make_database():
    tablenames=["links.csv", "movies.csv", "ratings.csv","tags.csv"] #list of tables needed to make database
...
def get_Avg_Movie_Rating():
    tablenames=["links.csv", "movies.csv", "ratings.csv","tags.csv"]
    dflist=[]
    for x in tablenames:
    ...
    query2= "SELECT m.title, round(avg(rating),2) FROM ratings r JOIN movies m ON CAST(r.movieId as INT)=CAST(m.movieId as INT) WHERE m.movieid IN (" #write sql query to fetch movie and average rating from database 
...
def getListByGenre():
    ...
    query2= "SELECT m.title, m.genres, round(avg(r.rating),2) FROM ratings r JOIN movies m ON CAST(r.movieId as INT)=CAST(m.movieId as INT) WHERE m.movieid IN (" #write SQL query to get the movie title, genre and average rating from our database
